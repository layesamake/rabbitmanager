...
import type { SujetType, ActionType, StatutTraitement, Traitement } from "../types";

export default function SanteEdit() {
  ...
  useEffect(() => {
    if (id) {
      db.traitements.get(id).then(t => {
        if (t) {
          setTraitement(t);
          setSujetType(t.sujetType);
          setSujetId(t.sujetId);
          setSujetNom(t.sujetNom || "");
          setTypeAction(t.typeAction);
          setNomProduit(t.nomProduit);
          setObjectif(t.objectif || "");
          setDatePrevue(t.datePrevue);
          setFrequence(t.frequence || "");
          setDuree(t.duree || "");
          setProchainRappel(t.prochainRappel || "");
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!nomProduit || !traitement) return;
    ...
    <div className="space-y-section pb-24">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h2 className="font-heading text-2xl font-semibold text-text-primary mt-1">Modifier traitement prévu</h2>
        </div>
      </div>
